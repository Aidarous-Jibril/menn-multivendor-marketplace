...
export const createProduct = createAsyncThunk(
  ...
  async (newForm, { rejectWithValue }) => {
    try {
      ...
      const { data } = await axios.post('/api/products/create-product', newForm, config);
      return data;
    } catch (error) {
...
export const vendorGetAllProducts = createAsyncThunk(
  ...
  async (vendorId, { rejectWithValue }) => {
    try {
      const { data } = await axios.get(`/api/products/${vendorId}/products`);
      return data.products; 
    } catch (error) {
      return rejectWithValue(error.response.data.message);
    }
...
export const fetchProductsBySubSubCategory = createAsyncThunk(
  ...
  async ({ subSubCategory }, { rejectWithValue }) => {
    try {
      const response = await axios.get(`/api/products?subSubCategory=${subSubCategory}`);
